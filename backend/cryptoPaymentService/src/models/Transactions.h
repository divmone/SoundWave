...
class Transactions
{
  public:
    ...
    /**  For column txhash  */
    ///Get the value of the column txhash, returns the default value if the column is null
    const std::string &getValueOfTxhash() const noexcept;
    ///Return a shared_ptr object pointing to the column const value, or an empty shared_ptr object if the column is null
    const std::shared_ptr<std::string> &getTxhash() const noexcept;
    ///Set the value of the column txhash
    void setTxhash(const std::string &pTxhash) noexcept;
