...
export default function ProductPage({ product, user, onNavigate, onLogout }) {
  ...
  const handleCryptoCheckout = async () => {
    ...
    if (!txHash.trim()) { setPurchaseError('Please enter transaction hash'); return; }
    setPurchaseError('');
    setPurchasing(true);
    try {
                  ...
                  {cryptoWallets.length > 0 && (
                    <div style={{ display: 'flex', gap: '0.75rem' }}>
                      ...
                      <button
                        ...
                        disabled={purchasing || !selectedWallet || !txHash.trim()}
                        style={{
                          ...
                          background: (selectedWallet && txHash.trim()) ? 'linear-gradient(135deg, rgba(220,140,0,0.95), rgba(255,190,0,0.95))' : 'var(--bg4)',
                          color: (selectedWallet && txHash.trim()) ? '#000' : 'var(--text3)',
                          opacity: (purchasing || !selectedWallet || !txHash.trim()) ? 0.6 : 1,
                          cursor: (purchasing || !selectedWallet || !txHash.trim()) ? 'not-allowed' : 'pointer',
                          transition: 'all 0.18s',
                        }}>
                        {purchasing ? 'Processing...' : 'Submit transaction'}
                      </button>
                    </div>
                  )}
                </>
